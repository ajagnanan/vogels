...
Table.prototype.mockCreate = function (item, options, callback) {
  var self = this;
  ...
  if (_.isArray(item)) {
    async.map(item, function (data, callback) {
      return internals.mockCreateItem(self, data, options, callback);
    }, callback);
  } else {
    return internals.mockCreateItem(self, item, options, callback);
  }
};

Table.prototype.mockUpdate = function (item, options, callback) {
  var self = this;
  ...
  if (_.isArray(item)) {
    async.map(item, function (data, callback) {
      return internals.mockUpdateItem(self, data, options, callback);
    }, callback);
  } else {
    return internals.mockUpdateItem(self, item, options, callback);
  }
};
...
internals.mockCreateItem = function (table, item, options, callback) {
  var self = table;
...
internals.mockUpdateItem = function (table, item, options, callback) {
  var self = table;

  var paramName = _.isString(self.schema.updatedAt) ? self.schema.updatedAt : 'updatedAt';

  if(self.schema.timestamps && self.schema.updatedAt !== false && !_.has(item, paramName)) {
    item.attrs[paramName] = new Date().toISOString();
  }

  return callback(null, item.attrs);
};
